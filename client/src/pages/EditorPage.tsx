...
export default function EditorPage() {
    ...
    return (
        <div className={`h-screen flex flex-col ${THEME.appBg}`}>
            <SEOHead
                title={`Editor - ${LANGUAGE_OPTIONS.find(l => l.key === lang)?.label ?? 'Code'}`}
                description="Write and run code instantly in your browser."
            />
            {/* Top Bar */}
            <div className={`flex items-center gap-3 p-3 ${THEME.headerBg} border-b ${THEME.divider}`}>
                <div className="text-lg font-semibold text-gray-100">{BRAND}</div>
                <Link to="/" className={`ml-2 px-3 py-1 rounded ${THEME.controlBg} text-gray-200 text-sm no-underline`}>Home</Link>
                <div className="flex items-center gap-2">
                    <label className="text-sm text-gray-300">Language</label>
                    <select
                        className={`${THEME.select} text-gray-200`}
                        value={lang}
                ...
                <div className="ml-2">
                    <button
                        className={`px-2 py-1 rounded ${THEME.controlBg} text-gray-200 text-sm`}
                ...
                <div className="ml-2">
                    <button
                        className="px-3 py-1 rounded bg-white/10 hover:bg-white/15 border border-white/10 text-gray-200 text-sm"
                        onClick={saveLocal}
                    >
                        Save
                    </button>
                ...
                <div className="ml-2">
                    <label className={`px-3 py-1 rounded ${THEME.controlBg} cursor-pointer text-gray-200 text-sm`}>
                        Import file
                        <input
                            type="file"
                            className="hidden"
            ...
            <div className="flex flex-1 min-h-0">
                ...
                <div className="w-1/2 flex flex-col min-h-0">
                    ...
                    <pre className={`flex-1 overflow-auto p-3 text-sm whitespace-pre-wrap ${THEME.preBg} text-gray-100`}>{outputText}</pre>
                </div>
            </div>
            ...
            <div className={`p-2 border-t ${THEME.divider} ${THEME.paneBg}`}>
                ...
                <textarea
                    className={`w-full h-24 p-2 rounded ${THEME.appBg} border ${THEME.divider} text-gray-200`}
                    value={stdin}
